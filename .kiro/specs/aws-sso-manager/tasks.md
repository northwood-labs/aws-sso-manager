...
* [ ] 1. Add test dependencies and shared test helpers
    * [~] 1.1 Add `pgregory.net/rapid` dependency to `go.mod`
        * Run `go get pgregory.net/rapid` to add the property-based testing library
        * _Requirements: Design Testing Strategy_
    * [~] 1.2 Create shared test generators in `cmd/testhelpers_test.go`
...
* [ ] 2. Implement behavior change: `--no-cache` fetch-then-delete ordering
    * [~] 2.1 Modify `list` command `--no-cache` logic in `cmd/list.go`
        ...
        * _Requirements: 3.18_
    * [~] 2.2 Write unit test for `--no-cache` fetch-then-delete ordering
...
* [ ] 3. Implement behavior change: lock file path
    * [ ] 3.1 Change lock file path in `cmd/lockutils.go`
    ...
    * [ ] 3.2 Write unit test for new lock file path
...
* [ ] 4. Implement behavior change: environment variable prefix
    * [ ] 4.1 Change env prefix from `ASV_` to `ASM_` in `cmd/root.go`
        * In `initializeConfig`, change `asvConfig.SetEnvPrefix("ASV")` to `asvConfig.SetEnvPrefix("ASM")`
        * _Requirements: 12.4_
    * [ ] 4.2 Write unit test for `ASM_` env prefix
...
* [ ] 5. Implement behavior change: verbose debug level with caller info
    * [ ] 5.1 Update verbose flag handling in `cmd/root.go` `PersistentPreRunE`
        ...
        * _Requirements: 12.5_
    * [ ] 5.2 Write unit test for verbose levels
        * Test that `-v` sets info, `-vv` sets debug, `-vvv` sets debug with `ReportCaller` enabled showing original filename and line number
        * _Requirements: 12.5_

* [ ] 6. Checkpoint - Verify behavior changes
    * Ensure all tests pass, ask the user if questions arise.

* [ ] 7. Property-based tests: URL normalization and data formatting
    * [ ] 7.1 Write property test for SSO Start URL normalization (Property 1)
    ...
    * [ ] 7.2 Write property test for account and role sorting (Property 2)
    ...
    * [ ] 7.3 Write property test for output formats containing all data (Property 3)
    ...
    * [ ] 7.4 Write property test for account and role filtering (Property 4)
...
* [ ] 8. Property-based tests: Lookup and cache
    * [ ] 8.1 Write property test for lookup index round trip (Property 5)
    ...
    * [ ] 8.2 Write property test for cache file path determinism (Property 10)
    ...
    * [ ] 8.3 Write property test for cache expiry detection (Property 11)
    ...
    * [ ] 8.4 Write property test for cache duration parsing (Property 12)
...
* [ ] 9. Property-based tests: Config validation and profile naming
    * [ ] 9.1 Write property test for managed block marker validation (Property 6)
    ...
    * [ ] 9.2 Write property test for profile name generation with pattern (Property 7)
    ...
    * [ ] 9.3 Write property test for substring match replacement (Property 8)
    ...
    * [ ] 9.4 Write property test for default profile name / toProfileToken idempotence (Property 9)
...
* [ ] 10. Property-based tests: Console, get, lookup, update
    * [ ] 10.1 Write property test for console URL account subdomain stripping (Property 13)
    ...
    * [ ] 10.2 Write property test for account ID validation (Property 14)
    ...
    * [ ] 10.3 Write property test for lookup account resolution (Property 15)
    ...
    * [ ] 10.4 Write property test for lookup role substring search (Property 16)
    ...
    * [ ] 10.5 Write property test for update managed section generation (Property 17)
...
* [ ] 11. Checkpoint - Verify property tests
    * Ensure all tests pass, ask the user if questions arise.

* [ ] 12. Unit tests for existing functionality
    * [ ] 12.1 Write unit tests for console URL stripping in `cmd/console_test.go`
        ...
        * _Requirements: 5.9_
    * [ ] 12.2 Write unit tests for command alias registration
